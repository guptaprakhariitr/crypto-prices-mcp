...
export interface Env {
  ...
  COINGECKO_BASE: string; COINGECKO_API_KEY?: string;
  ...
  CUSTOMER_PORTAL_RETURN_URL?: string; RESEND_API_KEY?: string; FROM_EMAIL?: string; PRODUCT_NAME?: string;
}

const SERVER_INFO = { name: "crypto-prices-mcp", version: "0.1.0" };
...
const LLMS_TXT = `# crypto-prices-mcp

> Live + historical cryptocurrency prices via CoinGecko's free API for AI agents.

## Tools
- crypto_prices(ids[], vs?)
- crypto_search(query)
- crypto_history(id, vs?, days?, interval?)
- crypto_trending()
- crypto_market_global()

Endpoint: https://crypto-prices-mcp.prakhar-cognizance.workers.dev/mcp
`;
const LANDING = `<!doctype html><html><head><meta charset="utf-8"><title>crypto-prices-mcp</title>
