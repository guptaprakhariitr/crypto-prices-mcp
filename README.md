...
[![Live](https://img.shields.io/badge/live-crypto-prices-mcp.atlasword.workers.dev-brightgreen.svg)](https://crypto-prices-mcp.atlasword.workers.dev)
...
|---|---|---|
| `crypto_prices` | `ids[], vs_currencies?` | Current prices for one or more coins. Pass CoinGecko ids (use crypto_search if you only have a symbol). |
| `crypto_search` | `query` | Search for a coin by name or symbol. Returns up to 10 matches with CoinGecko ids. |
| `crypto_history` | `id, days, interval?` | Historical price series. days = 1/7/14/30/90/180/365/max. interval = daily|hourly (paid tier). |
...
Get a key at **https://crypto-prices-mcp.atlasword.workers.dev/upgrade?tier=solo** (see [Getting an API key](#getting-an-api-key)).
